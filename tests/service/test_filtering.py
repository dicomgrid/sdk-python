...
class TestWithFilter:
    ...
    def test_filter_by_string_value(self, query):
        ...
        assert 'filter.field.equals' in query.request_data
        assert query.request_data['filter.field.equals'] == 'value'
    ...
    def test_filter_by_dt_with_fixed_timezone_offset(self, query):
        ...
        query.filter_by(Filter('field', FilterCondition.equals, value))
        assert 'filter.field.equals' in query.request_data
        assert query.request_data['filter.field.equals'] == str(
            value.replace(
                tzinfo=None,
            ),
        )

        assert 'filter.tz' in query.request_data
        assert query.request_data['filter.tz'] == '+03:00'
